"""Exporter contract: table -> xlsx, html -> docx, the tool.

Round-trips are asserted with the real readers (openpyxl / python-docx),
so "exported" means a mainstream library opens the file and finds the
content. The canonical `.slides.html` -> pptx exporter (`DeckPptxExporter`)
has its own test module: `test_deck_export.py`.
"""
