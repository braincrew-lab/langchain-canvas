...
class Canvas:
    ...
    def open_html(
        ...
        *,
        ...
        """
        artifact = Artifact(
            id=id or _new_id("page"), type="html", title=title, data={"html": ""}, meta=meta
        )
    ...
    def open_slides(
        ...
        *,
        ...
        norm = [
            s.model_dump(exclude_none=True) if isinstance(s, Slide) else s for s in (slides or [])
        ]
        artifact = Artifact(
            id=id or _new_id("deck"), type="slides", title=title, data={"slides": norm}
        )
        self._emit(CanvasCreate(artifact=artifact))
...
class DocumentHandle(_Handle):
    ...
    def replace(self, content: str) -> DocumentHandle:
        ...
        self._artifact = self._artifact.model_copy(
            update={
                "version": self._artifact.version + 1,
